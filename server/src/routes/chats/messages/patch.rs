use api_types::chats::messages::patch::{
    ApiChatsMessagesPatchRequest, ApiChatsMessagesPatchResponse,
};
...
pub async fn api_chats_messages_patch(
    Extension(user_id): Extension<i64>,
    State(pool): State<PgPool>,
    Json(payload): Json<ApiChatsMessagesPatchRequest>,
) -> impl IntoResponse {
    match update_message_impl(
        user_id,
        &pool,
...
pub async fn update_message_impl(
    ...
    message_id: Uuid,
    content: String,
) -> Result<ApiChatsMessagesPatchResponse, (StatusCode, String)> {
    ...
    match update_result {
        Ok(Some(row)) => {
            ...
            Ok(ApiChatsMessagesPatchResponse {
